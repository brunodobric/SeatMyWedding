...
/** Sortiraj stolove po broju u nazivu (Stol 1, Stol 2…), zatim abecedno. */
...
async function init() {
  ...
  // Auth prvo – da prijava radi čak i ako ostatak UI-a padne
  try {
    if (typeof setupAuth === 'function') setupAuth();
  } catch (e) {
    console.error('[init] setupAuth failed', e);
  }
  try {
    await loadAll();
  } catch (e) {
  ...
  try { setupNav(); } catch (e) { console.error('[init] setupNav', e); }
  try { setupHeader(); } catch (e) { console.error('[init] setupHeader', e); }
  try {
    if (typeof setupSync === 'function') setupSync();
  } catch (e) {
    console.error('[init] setupSync failed', e);
  }
  ...
  try {
    ...
    console.error('[init] first render failed', e);
  }
}
...
function renderTableList() {
  ...
  list.innerHTML = sortTablesByNumber(state.tables).map(t => {
...
function renderPregledContent() {
  ...
  if (pregledView === 'tables') {
    ...
    const tables = sortTablesByNumber(state.tables);
    container.innerHTML = tables.map(t => {
      ...
      return `<div class="pregled-table-card">
        ...
        <ol class="pregled-guest-list">${guests.map(g => {
          const cat = state.categories.find(c => c.id === g.categoryId);
          const note = g.notes && String(g.notes).trim() ? ` <span class="guest-note">(${esc(g.notes.trim())})</span>` : '';
          return `<li>${esc(g.name)}${cat ? ` <span class="cat-chip small" style="background:${cat.color}">${esc(cat.name)}</span>` : ''}${note}</li>`;
        }).join('')}</ol>
      </div>`;
    }).join('');
  } else if (pregledView === 'guests') {
    const tableOrder = new Map(sortTablesByNumber(state.tables).map((t, i) => [t.id, i]));
    const sorted = [...state.guests].sort((a, b) => {
      const ta = a.tableId != null ? (tableOrder.has(a.tableId) ? tableOrder.get(a.tableId) : 9999) : 10000;
      const tb = b.tableId != null ? (tableOrder.has(b.tableId) ? tableOrder.get(b.tableId) : 9999) : 10000;
    ...
    container.innerHTML = `<table class="pregled-table">
      <thead><tr><th>Gost</th><th>Kategorija</th><th>Stol</th><th>Mjesto</th><th>Status</th><th>Napomena</th></tr></thead>
      <tbody>${sorted.map(g => {
        const cat = state.categories.find(c => c.id === g.categoryId);
        const table = g.tableId ? state.tables.find(t => t.id === g.tableId) : null;
        return `<tr>
          ...
          <td>${g.seatIndex != null ? g.seatIndex + 1 : '—'}</td>
          <td>${g.status === 'assigned' ? '✔' : g.status === 'maybe' ? 'Možda' : '—'}</td>
          <td>${g.notes && String(g.notes).trim() ? esc(g.notes.trim()) : '—'}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
...
function setupPregledEvents() {
  ...
  document.getElementById('btn-export-csv')?.addEventListener('click', () => {
    const rows = [['Ime', 'Kategorija', 'Stol', 'Mjesto', 'Status', 'Napomena']];
    const tableOrder = new Map(sortTablesByNumber(state.tables).map((t, i) => [t.id, i]));
  ...
  document.getElementById('btn-export-json')?.addEventListener('click', () => {
    const data = JSON.stringify({
      ...
      tables: sortTablesByNumber(state.tables),
...
async function _appMain() {
  const steps = [
    setupModalClose, setupTableModal, setupGostiEvents, setupGuestModal,
    setupCategoryEditModal, setupAddCategoryBtn, setupPravilaEvents, setupRuleModal,
    setupRasporedSearch, setupAssignSeatModal, setupSeatOptionsModal,
    setupAutoSeating, setupSeatingResultModal, setupPregledEvents, setupSalaTabs
  ];
  for (const fn of steps) {
    try { fn(); } catch (e) { console.error('[boot]', fn.name || fn, e); }
  }
  try {
    await init();
  } catch (e) {
