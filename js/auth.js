...
function isAuthConfigured() {
  try {
    // Prefer explicit config object (survives if firebase.app() is briefly unavailable)
...
function setupAuth() {
  ...
  if (!isAuthConfigured()) {
    // Config stvarno nije postavljen – ostavi gumb vidljiv, prijavi grešku pri kliku
    console.warn(
      '[Firebase] Konfiguracija nije spremna. Provjeri firebase-config.js i da su Firebase skripte učitane.'
    );
    if (loginBtn) {
      loginBtn.style.display = '';
      loginBtn.addEventListener('click', () => {
        if (typeof showToast === 'function') {
          showToast('Prijava nije dostupna – Firebase nije učitan.', true);
        } else {
          alert('Prijava nije dostupna – Firebase nije učitan.');
        }
      });
    }
    return;
  ...
  // Ensure login button is visible when auth is ready
