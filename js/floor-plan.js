    ...
    seats.forEach((pos, idx) => {
      ...
      if (guest) {
        if (this.showFullNames) {
          ...
          // Ako je sjedište više "desno" od centra, anchor start; inače end
          ...
          // Blaga podloga radi čitljivosti
          txt.setAttribute('paint-order', 'stroke');
          txt.setAttribute('stroke', '#FDFBF7');
          txt.setAttribute('stroke-width', '3');
          txt.setAttribute('stroke-linejoin', 'round');
          txt.textContent = guest.name;
          seatG.appendChild(txt);
        } else {
          const initials = guest.name.split(/\s+/).filter(Boolean).map(n => n[0]).join('').slice(0, 2).toUpperCase();
          const txt = document.createElementNS('http://www.w3.org/2000/svg', 'text');
